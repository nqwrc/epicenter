...
const { createWhisperingRecipes } = await import('./recipes.svelte.js');

type RecipesTable = Parameters<typeof createWhisperingRecipes>[0]['table'];
...
test('a recipe stored before the trusted column survives the release that adds it', () => {
	...
	const recipes = createWhisperingRecipes({ table });
