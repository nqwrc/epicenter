...
const { createWhisperingAppRules } = await import('./app-rules.svelte.js');

type AppRulesTable = Parameters<typeof createWhisperingAppRules>[0]['table'];
...
test('a rule stored before the trusted column survives the release that adds it', () => {
	...
	const rules = createWhisperingAppRules({ table });
