...
test('a clean cursor paste is undoable, counted in graphemes', () => {
	lastDelivery.record({ text: 'hello', sinkKind: 'cursor', reach: 'output', pressedEnter: false });
...
test('an emoji and a combining mark each count as one backspace', () => {
	...
	lastDelivery.record({ text: '👨‍👩‍👧', sinkKind: 'cursor', reach: 'output', pressedEnter: false });
	...
	lastDelivery.record({ text: 'é', sinkKind: 'cursor', reach: 'output', pressedEnter: false });
	expect(lastDelivery.take()).toEqual({ graphemes: 1 });
});

test('nothing that skipped the keyboard is undoable', () => {
	lastDelivery.record({ text: 'hello', sinkKind: 'clipboard', reach: 'output', pressedEnter: false });
	expect(lastDelivery.take()).toBeNull();

	lastDelivery.record({ text: 'hello', sinkKind: 'ledger', reach: 'output', pressedEnter: false });
	expect(lastDelivery.take()).toBeNull();

	// A cursor write that could not paste left the text on the clipboard.
	lastDelivery.record({ text: 'hello', sinkKind: 'cursor', reach: 'clipboard', pressedEnter: false });
	expect(lastDelivery.take()).toBeNull();
});
